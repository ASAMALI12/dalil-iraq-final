...
const ADMIN_PHONE = process.env.IRAQ_ADMIN_PHONE || "";
...
app.use(express.urlencoded({ extended: true, limit: "50mb" }));
  ...
  app.post("/api/admin/login", (req: Request, res: Response) => {
    ...
    if (!username || !password || !phone) {
      return res.status(400).json({ success: false, error: "رقم الهاتف واسم المستخدم وكلمة المرور مطلوبة." });
    }
    ...
    const cleanAdminPhone = normalizeIraqPhone(phone);
    const cleanExpectedPhone = normalizeIraqPhone(ADMIN_PHONE);
    const isValidPhone = Boolean(cleanExpectedPhone && cleanAdminPhone === cleanExpectedPhone);
  ...
  app.post("/api/claim/request-otp", async (req: Request, res: Response) => {
    try {
      let { storeId, storeName, storePhone, applicantName, applicantPhone } = req.body;
      ...
      // Always resolve the authoritative store phone from the database; never trust a client-supplied storePhone.
      const sbForClaim = getServerSupabase();
      if (!sbForClaim) return res.status(503).json({ success: false, error: "قاعدة بيانات المتاجر غير متاحة حالياً." });
      const { data: authoritativeStore, error: authoritativeStoreError } = await sbForClaim.from('stores').select('id,name,phone,is_claimed,claim_status').eq('id', storeId).maybeSingle();
      if (authoritativeStoreError) throw authoritativeStoreError;
      if (!authoritativeStore) return res.status(404).json({ success: false, error: "المتجر غير موجود." });
      if (authoritativeStore.is_claimed && authoritativeStore.claim_status === 'verified') return res.status(409).json({ success: false, error: "هذا المتجر موثق مسبقاً." });
      storePhone = authoritativeStore.phone;
      storeName = authoritativeStore.name || storeName;
  ...
  // Paid advertisement workflow: verified store owner -> payment receipt -> admin approval -> publish
  app.get("/api/ad-payment-settings", (_req: Request, res: Response) => {
    res.json({
      success: true,
      zaincashNumber: process.env.IRAQ_ADMIN_PHONE || "",
      mastercardAccount: process.env.IRAQ_MASTER_CARD_ACCOUNT || "",
      currency: "IQD",
      plans: {
        iraq: 25000,
        governorate: 15000,
  ...
  app.post("/api/ad-payment-requests", async (req: Request, res: Response) => {
    try {
      const b = req.body || {};
      const sb = getServerSupabase();
      if (!sb) return res.status(503).json({ success: false, error: "خدمة قاعدة البيانات غير متاحة حالياً." });
      if (!b.storeId || !b.headline || !b.transferReference || !b.receiptData || !b.paymentMethod) {
        return res.status(400).json({ success: false, error: "بيانات الإعلان والدفع والوصل مطلوبة." });
      }
      if (!['zaincash', 'mastercard'].includes(b.paymentMethod)) {
        return res.status(400).json({ success: false, error: "طريقة الدفع غير صالحة." });
      }
      if (typeof b.receiptData !== 'string' || b.receiptData.length > 7_000_000) {
        return res.status(400).json({ success: false, error: "حجم وصل الدفع كبير جداً." });
      }
      const { data: store, error: storeError } = await sb.from('stores').select('id,name,is_claimed,claim_status,phone,whatsapp').eq('id', b.storeId).maybeSingle();
      if (storeError) throw storeError;
      if (!store) return res.status(404).json({ success: false, error: "المتجر غير موجود." });
      if (!store.is_claimed || store.claim_status !== 'verified') {
        return res.status(403).json({ success: false, error: "يجب توثيق المتجر أولاً قبل نشر إعلان مدفوع." });
      }
      const amount = Number(b.amount);
      if (!Number.isFinite(amount) || amount <= 0) return res.status(400).json({ success: false, error: "مبلغ الإعلان غير صالح." });
      const durationDays = Number(b.durationDays);
      if (!Number.isInteger(durationDays) || durationDays < 1 || durationDays > 90) return res.status(400).json({ success: false, error: "مدة الإعلان غير صالحة." });
      const id = `adpay_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`;
      const row = {
        id, store_id: String(store.id), store_name: store.name || b.storeName || '',
        plan_scope: b.planScope || 'district', governorate_id: b.governorateId || null,
        district_id: b.districtId || null, category_id: b.categoryId || null,
        headline: String(b.headline).slice(0, 160), description: String(b.description || '').slice(0, 2000),
        image_url: b.imageUrl || null, phone: store.phone || b.phone || null, whatsapp: store.whatsapp || b.whatsapp || null,
        offer_badge: b.offerBadge || null, duration_days: durationDays, amount,
        payment_method: b.paymentMethod, recipient_account: b.paymentMethod === 'zaincash' ? (process.env.IRAQ_ADMIN_PHONE || '') : (process.env.IRAQ_MASTER_CARD_ACCOUNT || ''),
        transfer_reference: String(b.transferReference).slice(0, 120), receipt_data: b.receiptData,
        text_color: b.textColor || '#ffffff', background_color: b.backgroundColor || '#0f172a',
        font_family: b.fontFamily || 'inherit', media_type: b.mediaType || 'image',
        media_items: Array.isArray(b.mediaItems) ? b.mediaItems.slice(0, 5) : [], design: b.design || {}, status: 'pending'
      };
      const { data, error } = await sb.from('ad_payment_requests').insert(row).select('id,status,created_at').single();
      if (error) throw error;
      return res.json({ success: true, request: data, message: "تم إرسال طلب الدفع بنجاح، وسيظهر الإعلان بعد موافقة المدير على الوصل." });
    } catch (err: any) {
      console.error('ad-payment-requests create error', err);
      return res.status(500).json({ success: false, error: err?.message || 'تعذر إنشاء طلب الدفع.' });
    }
  });

  app.get("/api/admin/ad-payment-requests", requireAdminAuth, async (_req: Request, res: Response) => {
    const sb = getServerSupabase();
    if (!sb) return res.status(503).json({ success: false, error: "قاعدة البيانات غير متاحة." });
    const { data, error } = await sb.from('ad_payment_requests').select('*').order('created_at', { ascending: false });
    if (error) return res.status(500).json({ success: false, error: error.message });
    return res.json({ success: true, requests: data || [] });
  });

  app.post("/api/admin/ad-payment-requests/:id/review", requireAdminAuth, async (req: Request, res: Response) => {
    try {
      const sb = getServerSupabase();
      if (!sb) return res.status(503).json({ success: false, error: "قاعدة البيانات غير متاحة." });
      const { id } = req.params;
      const { status, adminNotes } = req.body || {};
      if (!['approved', 'rejected'].includes(status)) return res.status(400).json({ success: false, error: "حالة المراجعة غير صالحة." });
      const { data: request, error: re } = await sb.from('ad_payment_requests').select('*').eq('id', id).maybeSingle();
      if (re) throw re;
      if (!request) return res.status(404).json({ success: false, error: "طلب الدفع غير موجود." });
      if (request.status !== 'pending') return res.status(409).json({ success: false, error: "تمت مراجعة هذا الطلب مسبقاً." });
      if (status === 'rejected') {
        const { data, error } = await sb.from('ad_payment_requests').update({ status, admin_notes: adminNotes || null, reviewed_at: new Date().toISOString() }).eq('id', id).select().single();
        if (error) throw error;
        return res.json({ success: true, request: data });
      }
      const startsAt = new Date();
      const expiresAt = new Date(startsAt.getTime() + Number(request.duration_days) * 86400000);
      const ad = {
        id: `ad_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`, store_id: request.store_id,
        title: request.headline, description: request.description, image_url: request.image_url,
        target_governorate_id: request.governorate_id, target_district_id: request.district_id,
        category_id: request.category_id, phone: request.phone, whatsapp: request.whatsapp,
        badge: request.offer_badge, duration_days: request.duration_days, price: request.amount,
        payment_method: request.payment_method, reference_number: request.transfer_reference,
        status: 'active', is_active: true, starts_at: startsAt.toISOString(), expires_at: expiresAt.toISOString(),
        text_color: request.text_color, background_color: request.background_color, font_family: request.font_family,
        media_type: request.media_type, media_items: request.media_items, design: request.design,
        created_by: 'admin-approved-payment'
      };
      const { data: createdAd, error: ae } = await sb.from('advertisements').insert(ad).select().single();
      if (ae) throw ae;
      const { data: updated, error: ue } = await sb.from('ad_payment_requests').update({ status: 'approved', admin_notes: adminNotes || null, reviewed_at: startsAt.toISOString(), expires_at: expiresAt.toISOString() }).eq('id', id).select().single();
      if (ue) throw ue;
      return res.json({ success: true, request: updated, advertisement: createdAd });
    } catch (err: any) {
      console.error('ad-payment review error', err);
      return res.status(500).json({ success: false, error: err?.message || 'تعذر مراجعة طلب الدفع.' });
    }
  });
  ...
  app.get("/api/advertisements", async (req: Request, res: Response) => {
    ...
    if (sb) {
      try {
        let q = sb.from("advertisements").select("*").eq("is_active", true).order("created_at", { ascending: false });
        const gov = String(req.query.governorateId || '');
        const dist = String(req.query.districtId || '');
        if (gov && gov !== 'all') q = q.or(`target_governorate_id.eq.${gov},target_governorate_id.is.null`);
        if (dist && dist !== 'all') q = q.or(`target_district_id.eq.${dist},target_district_id.is.null`);
        const { data, error } = await q;
        if (!error && Array.isArray(data)) {
          const now = Date.now();
          const active = data.filter((a:any) => !a.expires_at || new Date(a.expires_at).getTime() > now && (!a.starts_at || new Date(a.starts_at).getTime() <= now));
          return res.json({ success: true, count: active.length, ads: active });
        }
      } catch (e) {
...
-- 10. جدول طلبات دفع الإعلانات (Manual Zain Cash / Mastercard)
...
-- 11. جدول المحافظ (Wallets) - legacy/internal only; no longer used for ad payments
