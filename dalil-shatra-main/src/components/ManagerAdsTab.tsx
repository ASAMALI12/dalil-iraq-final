...
import { safeApiFetch, getApiUrl } from '../utils/apiClient';
...
export const ManagerAdsTab: React.FC = () => {
  ...
  const loadPaymentRequests = async () => {
    ...
    const r = await safeApiFetch('/api/admin/ad-payment-requests', { headers: { Authorization: `Bearer ${token}` } }, 15000);
    setLoadingPayments(false);
    if (r.success && Array.isArray(r.data?.requests)) setPaymentRequests(r.data.requests);
  };
  const reviewPayment = async (id: string, status: 'approved'|'rejected') => {
    const token = sessionStorage.getItem('iraq_admin_token');
    if (!token) return;
    const r = await safeApiFetch(`/api/admin/ad-payment-requests/${encodeURIComponent(id)}/review`, { method:'POST', headers:{ Authorization:`Bearer ${token}`,'Content-Type':'application/json' }, body:JSON.stringify({status}) }, 20000);
    if (r.success) { setActionSuccessMsg(status==='approved'?'تم اعتماد الدفع ونشر الإعلان.':'تم رفض طلب الدفع.'); loadPaymentRequests(); }
    else setActionSuccessMsg(r.error || 'تعذر تنفيذ العملية.');
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-sky-800 bg-slate-950 p-4 space-y-3">
        <div className="flex items-center justify-between"><div><h4 className="text-white font-bold">طلبات الدفع بانتظار المراجعة</h4><p className="text-xs text-slate-400">لا ينشر إعلان صاحب المتجر قبل موافقة المدير.</p></div><button onClick={loadPaymentRequests} className="rounded-lg bg-slate-800 px-3 py-2 text-xs font-bold text-white">{loadingPayments?'جاري التحديث...':'تحديث'}</button></div>
        {paymentRequests.length===0 ? <div className="text-xs text-slate-500">لا توجد طلبات محملة. اضغط تحديث.</div> : <div className="space-y-3">{paymentRequests.filter(x=>x.status==='pending').map(x=><div key={x.id} className="rounded-xl border border-slate-800 bg-slate-900 p-3 text-right"><div className="font-bold text-white">{x.store_name} — {x.headline}</div><div className="text-xs text-slate-400 mt-1">{Number(x.amount).toLocaleString('ar-IQ')} د.ع • {x.payment_method==='zaincash'?'زين كاش':'Mastercard'} • {x.transfer_reference}</div>{x.receipt_data && <img src={x.receipt_data} className="mt-2 max-h-48 max-w-full rounded-lg object-contain bg-white"/>}<div className="flex gap-2 mt-3"><button onClick={()=>reviewPayment(x.id,'approved')} className="flex-1 rounded-lg bg-emerald-600 py-2 text-xs font-bold text-white">اعتماد ونشر</button><button onClick={()=>reviewPayment(x.id,'rejected')} className="flex-1 rounded-lg bg-rose-600 py-2 text-xs font-bold text-white">رفض</button></div></div>)}</div>}
      </div>
