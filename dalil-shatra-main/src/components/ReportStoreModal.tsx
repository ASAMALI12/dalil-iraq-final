...
import { fetchAdPaymentSettings } from '../services/adPaymentService';
...
export const ReportStoreModal: React.FC<ReportStoreModalProps> = ({
  ...
  useEffect(() => { fetchAdPaymentSettings().then((x:any) => { if (x?.zaincashNumber) setAdminPhone(String(x.zaincashNumber).replace(/^\+/, '')); }); }, []);
...
يرجى المتابعة والتحقق مع فائق الشكر.`;
  ...
  const handleSendViaWhatsApp = () => {
    ...
    handleSubmitReport();
    // Open WhatsApp
    const url = `https://wa.me/${adminPhone.replace(/^0/, '964')}?text=${generateWhatsAppMessage()}`;
          ...
          {isSubmitted ? (
            <div className="py-8 text-center space-y-4 animate-in zoom-in-95 duration-200">
              ...
              <div className="pt-3 flex flex-col sm:flex-row items-center justify-center gap-2">
                ...
                <a
                  href={`https://wa.me/${adminPhone.replace(/^0/, '964')}?text=${generateWhatsAppMessage()}`}
                  target="_blank"
