...
export const WalletProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [managerCredentials] = useState<ManagerCredentials>({ phone: '', username: '', password: '' });
  ...
  const loginManager = (
    ...
    const token = sessionStorage.getItem('iraq_admin_token');
    if (!token) return { success: false, message: 'يجب التحقق من بيانات المدير عبر الخادم.' };
  ...
  const updateManagerCredentials = (currentPassword: string, newPhone: string, newUsername: string, newPassword: string): { success: boolean; message?: string } => ({ success: false, message: 'تغيير بيانات المدير يتم من إعدادات الخادم الآمنة فقط.' });
